...
async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='๖ۣۜζ͜͡VIRTUAL REFORMED', type=1))
...
async def on_member_remove(member):
    ...
    person_count = len([member for member in member.server.members if not member.bot])
    bot_count = len([member for member in member.server.members if member.bot])
    ...
    await client.edit_channel(memberchannel, name= f"Member Count: {format(str(member.server.member_count))}")
    await client.edit_channel(userchannel, name= f"User Count: {person_count}")
    await client.edit_channel(botchannel, name= f"Bot Count: {bot_count}")
...
async def rule4(ctx):
    ...
    await client.send_typing(ctx.message.channel)
    await client.send_message(ctx.message.channel, rule4)
...
async def suggest(ctx, *, msg: str):
    ...
    await client.send_message(ctx.message.channel, f"**{ctx.message.author.display_name}, We've recieved your suggestion and we'll take action about it soon... Again, Thank you for suggesting and have a nice day...**")
...
@client.command(pass_context = True)
async def hlo(ctx):
    await client.send_message(ctx.message.channel, f"{ctx.message.author.mention}, How you're doing?")
